...
$::debianflag             = 0;
$::MYSQL_PORT             = undef;  # Define a new variable to store the port
$::XCATMYSQLADMIN_PW      = undef;  # Variable for xcatadmin MySQL password
$::XCATMYSQLROOT_PW       = undef;  # Variable for root MySQL password
...
if (
    !GetOptions(
        ...
        'v|version'               => \$::VERSION,
        'V|verbose'               => \$::VERBOSE,
        'p|port=i'                => \$::MYSQL_PORT,       # Add the new port option
        'XCATMYSQLADMIN_PW=s'     => \$::XCATMYSQLADMIN_PW,  # Add xcatadmin MySQL password option
        'XCATMYSQLROOT_PW=s'      => \$::XCATMYSQLROOT_PW   # Add root MySQL password option
    )
  )
{
    &usage;
...
# Assign $::mysql_port to $MYSQL_PORT for global access if needed
our $MYSQL_PORT = $::MYSQL_PORT;
...
if (defined($::MYSQL_PORT)) {
    if ($::MYSQL_PORT < 1 || $::MYSQL_PORT > 65535) {
        xCAT::MsgUtils->message("E", "Invalid port number. Please specify a valid port between 1 and 65535.");
        exit 1;
    }
    xCAT::MsgUtils->message("I", "Using port: $::MYSQL_PORT");
}
...
sub validate_password {
    ...
    if ($password =~ m/[^a-zA-Z0-9]/) {
        my $warning = "The $role password is not alpha-numeric.";
        xCAT::MsgUtils->message("E", $warning);
        exit 1;
    }
}

# Updated subcode for root and xcatadmin MySQL passwords
if ((($odbconly == 0) && ($::xcatrunningmysql == 0)) || $::UPDATE || $::SETUPLL) {
    # Get xcatadmin password
    if ($::XCATMYSQLADMIN_PW) {
        $::adminpassword = $::XCATMYSQLADMIN_PW;
        validate_password($::adminpassword, "xcatadmin");
    } elsif ($ENV{'XCATMYSQLADMIN_PW'}) {
        $::adminpassword = $ENV{'XCATMYSQLADMIN_PW'};
        validate_password($::adminpassword, "xcatadmin");
    } else {
        my $msg = "Input the alpha-numeric password for xcatadmin in the MySQL database: ";
    ...
    # Get root password
    if ($::XCATMYSQLROOT_PW) {
        $::rootpassword = $::XCATMYSQLROOT_PW;
        validate_password($::rootpassword, "root");
    } elsif ($ENV{'XCATMYSQLROOT_PW'}) {
...
# Additional checks for flags like -i, -u, etc.
if ((!($::INIT)) && (!($::UPDATE)) && (!($::SETUPODBC)) && (!($::SETUPLL))) {
    xCAT::MsgUtils->message("I", "Either -i or -u or -o flag must be chosen");
    &usage;
    exit(1);
...
if (($::INIT) && ($::xcatrunningmysql == 0))
{
    ...
    if ($::mysqlrunning == 0)
    {
        ...
        &setupmysqlroot;
        #
        # Grant permissions to xCAT user
        #
        &grant_permissions_to_xcat;
    }
...
sub mysqlstart {
    ...
    if ($::osname eq 'AIX') {
        ...
        $cmd = $cmd2;
        $cmd .= "/usr/bin/mysqld --user=mysql --basedir=$::installdir --datadir=/var/lib/mysql --log-error=/var/lib/mysql/$hostname.err --pid-file=/var/lib/mysql/$hostname.pid --socket=/tmp/mysql.sock --port=$MYSQL_PORT &";
        $ret = xCAT::Utils->runcmd($cmd, 0);
    } else {
        if ($::MariaDB == 1) {    # running MariaDB
            if ($::linuxos =~ /rh.*|ol.*|rocky.*|alma.*|sles15.*/) {
                $cmd = "/usr/bin/mariadbd --user=mysql --basedir=/usr --datadir=/var/lib/mysql --socket=/var/lib/mysql/mysql.sock --port=$MYSQL_PORT &";
            } else {              # SLES
                $cmd = "/usr/bin/mysqld --user=mysql --basedir=/usr --datadir=/var/lib/mysql --socket=/var/lib/mysql/mysql.sock --port=$MYSQL_PORT &";
            }
        } else {    # it is mysql
            if ($::linuxos =~ /rh.*|ol.*|rocky.*|alma.*/) {
                $cmd = "nohup /usr/bin/mysqld_safe --user=mysql --basedir=/usr --datadir=/var/lib/mysql --socket=/var/lib/mysql/mysql.sock --port=$MYSQL_PORT > /dev/null 2>&1 &";
            } else {              # SLES
                $cmd = "/usr/bin/mysqld --user=mysql --basedir=/usr --datadir=/var/lib/mysql --socket=/var/lib/mysql/mysql.sock --port=$MYSQL_PORT &";
            }
        }
...
# Check for process running
    ...
    if (grep(/$mysql_process_name/, @output)) {
        ...
        my $cmd_start = "nohup /usr/bin/mysqld_safe --user=mysql --basedir=/usr --datadir=/var/lib/mysql --socket=/var/lib/mysql/mysql.sock --port=$MYSQL_PORT > /dev/null 2>&1 &";
        system($cmd_start);
...
=head3   grant_permissions_to_xcat


    Setup for xcatadmin permission on xcatdb

=cut

#-----------------------------------------------------------------------------
sub grant_permissions_to_xcat {
    # Get system hostname
    my $hostname = hostname;
    # Grant permissions to the xCAT user
    print "Granting permissions to xcatadmin...\n";
    ...
    if ($::osname eq 'AIX') {
        my $cmd2 = "ulimit -n unlimited; ulimit -m unlimited; ulimit -d unlimited; ulimit -f unlimited; ulimit -s unlimited;";
        $cmd = $cmd2;
        $cmd .= "$::installdir/bin/mysql -u root -p$::rootpassword -e \"";
        $cmd .= "GRANT ALL PRIVILEGES ON xcatdb.* TO 'xcatadmin'\@'$hostname' IDENTIFIED BY '$::adminpassword';";
        $cmd .= "GRANT ALL PRIVILEGES ON *.* TO 'xcatadmin'\@'%' IDENTIFIED BY '$::adminpassword';";
        $cmd .= "FLUSH PRIVILEGES;\"";
    } else {
        $cmd = "/usr/bin/mysql -u root -p$::rootpassword -e \"";
        $cmd .= "GRANT ALL PRIVILEGES ON xcatdb.* TO 'xcatadmin'\@'$hostname' IDENTIFIED BY '$::adminpassword';";
        $cmd .= "GRANT ALL PRIVILEGES ON *.* TO 'xcatadmin'\@'%' IDENTIFIED BY '$::adminpassword';";
        $cmd .= "FLUSH PRIVILEGES;\"";
    }
    ...
    if ($::RUNCMD_RC == 0) {
        ...
        print "Permissions granted successfully to xcatadmin.\n";
        return;
    }
    ...
    xCAT::MsgUtils->message(
        "E",
        "Failed to grant permissions to xCAT user."
    );
    exit 1;
}
...

{
    ...
    my $mysqlentry =
      "mysql:dbname=xcatdb;host=$::MN;port=$MYSQL_PORT|xcatadmin|$::adminpassword";
