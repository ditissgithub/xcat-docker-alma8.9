...
# Prompt for inputs
read -e -p "Enter The Subnet Prefix of Network (Valid range: 18-24): " subnet_prefix
...
read -e -p "Enter Private Network Address (Starting Pvt_IP Address) Of The Subnet: " pv_net_address
read -e -p "Enter BMC Network Address (Starting BMC_IP Address) Of The Subnet: " bmc_net_address
read -e -p "Enter IB Network Address (Starting IB_IP Address) Of The Subnet: " ib_net_address
read -e -p "Enter The Prefix Value For Compute Node (For ex: rbcn or rpcn or cn): " prefix
read -e -p "Enter The Node Type To Add In Groups(For ex: compute,hm,gpu): " node_type
read -e -p "Enter The Start Compute Node No: " start_node_no
read -e -p "Enter The Last Compute Node No: " last_node_no
read -e -p "Enter The MAC file to add node definition (For ex: compute_mac.txt,gpu_mac.txt,hm_mac.txt): " mac_file
# Check if mac.txt file exists
if [ ! -f "$mac_file" ]; then
    echo "Error: $mac_file file not found!"
    exit 1
fi
...
define_prefix() {
    j=$1
    if [ -z "$mac" ]; then
        echo "Error: MAC address not found for node $j in $mac_file"
        exit 1
    fi
...
for ((block = 0; block <= num_full_blocks; block++)); do
    ...
    for ((i = 1; i <= max_nodes_in_block; i++)); do
        ...
        U=$(($block + $(echo $pv_net_address | awk -F '.' '{print $4}')))
        V=$(($block + $(echo $bmc_net_address | awk -F '.' '{print $4}')))
        W=$(($block + $(echo $ib_net_address | awk -F '.' '{print $4}')))
        A=$(($i + $U - 1))
        B=$(($i + $V - 1))
        C=$(($i + $W - 1))
        pvt_ip_network="${pvt_ip_network_var}.$R.$A"
        bmc_ip_network="${bmc_ip_network_var}.$S.$B"
        ib_ip_network="${ib_ip_network_var}.$T.$C"
